...
def send_message(user_message, history, user_id, conversation_id, username):
    ...
    if username and conversation_id:
        if not users_db[username]["conversations"][conversation_id].get(
            "title_set", False
        ):
                    ...
                    if line:
                        ...
                        if json_str.startswith("{") and json_str.endswith("}"):
                            try:
                                ...
                                if (
                                    response_data.get("choices", [{}])[0].get(
                                        "finish_reason"
                                    )
                                    == "stop"
                                ):
...
""",
        ...
        with gr.Row(elem_classes="header"):
            ...
            with gr.Row():
                ...
                new_conv_button = gr.Button(
                    "新建会话", elem_classes="header-btn", variant="secondary"
                )
                # 定义历史会话按钮
                history_button = gr.Button(
                    "历史会话", elem_classes="header-btn", variant="secondary"
                )
                # 定义退出登录按钮
                logout_button = gr.Button(
                    "退出登录", elem_classes="header-btn", variant="secondary"
                )
    ...
    with gr.Column(visible=False, elem_classes="modal") as history_modal:
        ...
        conv_dropdown = gr.Dropdown(
            label="选择历史会话", choices=["请选择历史会话"], value="请选择历史会话"
        )
        # 定义加载会话按钮
    ...
    # 定义隐藏注册弹窗的函数
    def hide_register_modal():
        return gr.update(visible=False)

    # 定义显示历史弹窗的函数，并更新会话列表
    def show_history_modal(username):
        return gr.update(visible=True), gr.update(
            choices=get_conversation_list(username), value="请选择历史会话"
        )
    ...
    login_button.click(
        ...
        # 根据登录状态切换页面显示
        lambda logged: (gr.update(visible=not logged), gr.update(visible=logged)),
        [logged_in],
        ...
        # 加载当前会话历史
        lambda username, conv_id: (
            users_db[username]["conversations"][conv_id]["history"]
            if username and conv_id
            else []
        ),
        [current_user, current_conversation],
        chatbot_history,
    ).then(
        # 更新会话标题
        lambda username, conv_id: (
            users_db[username]["conversations"][conv_id].get("title", "创建新的聊天")
            if username and conv_id
            else "创建新的聊天"
        ),
        [current_user, current_conversation],
    ...
    history_button.click(
        show_history_modal, [current_user], [history_modal, conv_dropdown]
    )
    ...
    new_conv_button.click(
        new_conversation, [current_user], [login_output, current_conversation]
    ).then(
        # 清空聊天对话框
        lambda: [],
        None,
    ...
    load_conv_button.click(
        load_conversation, [current_user, conv_dropdown], chatbot
    ).then(
        # 更新当前会话 ID
        lambda user, conv: extract_conversation_id(conv, user),
        [current_user, conv_dropdown],
        current_conversation,
    ).then(
        # 更新会话标题
        lambda username, conv: (
            users_db[username]["conversations"][
                extract_conversation_id(conv, username)
            ].get("title", "创建新的聊天")
            if username and conv
            else "创建新的聊天"
        ),
        [current_user, conv_dropdown],
        conversation_title,
    ...
    send.click(
        send_message,
        [message, chatbot_history, current_user_id, current_conversation, current_user],
        [chatbot, chatbot_history, conversation_title],
    ).then(
        # 更新聊天历史
        update_history,
        [chatbot, chatbot_history, current_user, current_conversation],
        chatbot_history,
    ).then(
        # 更新会话标题
        lambda username, conv_id: (
